...
function ImageRow ({images, offset = 0}){
    //we are going to double the images so the row is wide emough to never show gaps
    const doubled = [...images, ...images];
    return(
        <div className="carrousel-row" style={{transform: `translate3d(${offset}px, 0, 0)`}}>
...
export default function FeaturesSection(){
    ...
    useEffect(()=>{
        const handleScroll = ()=>{
            if(!sectionRef.cuurent) return;
            const rect = sectionRef.current.getBoundClientRect();
            ...
            //each row moves at diferent speeds/directions based on scroll progress
            //scale range to virw port width so it works on all screen sizes
